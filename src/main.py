...
history_payment_dir = os.path.join(config.BASE_DIR, "history_payment")
if os.path.exists(history_payment_dir) is False:
    os.mkdir(history_payment_dir)
...
def start_message(message):
    ...
    bot.send_message(message.chat.id, "<b>Добро пожаловать в {}.</b>".format(config.BOT_NAME), reply_markup=markup,
                     parse_mode="HTML")
...
def messages(message):
    ...
    if message.text == "Обратная связь":
        ...
        bot.send_message(chat_id,
                         "Возникли <b>вопросы?</b>\nНеобходима большая <b>партия снюса?</b>\n\nВоспользуйтесь кнопкой "
                         "ниже для связи с менеджером.",
        ...
        bot.send_message(chat_id,
                         "<b>Для проверки оплаты перейдите по ссылке ниже, и после оплаты нажмите ОПЛАТИЛ</b>\n\n<i>Сумма оплаты: 1 руб.</i>",
        ...
        markup.row("Обратная связь")
        markup.row("Тестовая оплата")
        bot.send_message(chat_id, "<b>Добро пожаловать в {}.</b>".format(config.BOT_NAME), parse_mode="HTML",
                         reply_markup=markup)
    ...
    # оплата
        ...
        if sum < "200":
            bot.send_message(chat_id,
                             "Произошла ошибка. Попробуйте снова, или через пару минут.\n\nНаши специалисты уже начали поиск проблемы.",
                             parse_mode="HTML")
        else:
            ...
            markup.row(button1)
            bot.send_message(chat_id,
                             "Ваш заказ:\n<i>{}\n{}\n\n</i>Сумма оплаты: {}{}".format(splitter[0], splitter[1], sum,
                                                                                      config.A_T), parse_mode="HTML",
                             reply_markup=markup)
    ...
    if call.data == "check":
        ...
        if str(last_payment["sum"]) == str(sum):
            bot.send_message(chat_id,
                             "<b>Ваша оплата успешно получена!</b>\n\n Полученный нами адрес:\n<i>{}</i>".format(
                                 last_payment["description"]), parse_mode="HTML")
            send_in_group(
                "Получена оплата в размере - {}р от юзера @{} с chat_id {}".format(sum, call.message.chat.username,
                                                                                   chat_id))
        ...
        if str(last_payment["sum"]) == str(sum):
            bot.send_message(chat_id,
                             "<b>Ваша оплата успешно получена!</b>\n\n Полученный нами адрес:\n<i>{}</i>".format(
                                 last_payment["description"]), parse_mode="HTML")
            send_in_group("Получена тестовая оплата в размере - {}р от юзера @{} с chat_id {}".format(sum,
